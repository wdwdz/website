...
The column mean s are calculated by assigning interger scores, i.e. access score is ordinary.
It means the access score are correlated with the neighborhood types.


```{r}
Q1<-xtabs(~access.score + Q7.7,data=Q7.7_access)
#write.table(Q1,file = "ctable_access_neighbor.csv",row.names = TRUE,sep=",")
CMHtest(Q1)

```


i.	 is access a moderator b/w use or time outdoors and neighborhood type

```{r}
df.chi<- df.chi %>% 
    cbind("neighbortype"=as.factor(Q7.7_access$Q7.7)) 
Q2<-xtabs(~neighbortype+time.daysperweek+access,data = df.chi)
#write.table(Q2,file = "ctable_time_neighbor_access.csv",row.names = TRUE,sep=",")
...
```
a.	Does access relate to use (Q2.4) – correlation between TOTAL TIME outdoors (Fall 2020) and: [TABLE]
...
```
b.	Compare each above to time outdoors since covid emerged (Q2.8) and in Fall 2019 (Q2.10)
```{r}

```
c.	Does use (ie time outdoors) correlate with any of the following: [TABLE]
i.	Gender (Q7.2)
...
```
d.	Changes in use of spaces pre to post covid – within and across subjects
```{r}
