...
a. CMH correlation statistic ("cor"), treating both factors as ordered. For a given statum, with equally spaced row and column scores, this CMH statistic reduces to (n-1) r^2, where r is the Pearson correlation between X and Y. With "midrank" scores, this CMH statistic is analogous to (n-1) r_S^2, using the Spearman rank correlation.
b. The ANOVA (row mean scores and column mean scores) statistics, treat the columns and rows respectively as ordinal, and are sensitive to mean shifts over columns or rows. These are transforms of the F statistics from one-way ANOVAs with equally spaced scores and to Kruskal-Wallis tests with "midrank" scores.
c. general association statistic treat both factors as unordered, and give a test closely related to the Pearson χ^2 test. When there is more than one stratum, the overall general CMH statistic gives a stratum-adjusted Pearson χ^2, equivalent to what is calculated by mantelhaen.test.
When the value in any cell is smaller than 5, Fisher test is recommended. 
...
Sieve diagram
Riedwyl and Schüpbach (1983, 1994) proposed a sieve diagram (later called a parquet diagram ) based on this principle. In this display the area of each rectangle is proportional to expected frequency and observed frequency is shown by the number of squares in each rectangle. Hence, the difference between observed and expected frequency appears as the density of shading, using color to indicate whether the deviation from independence is positive or negative. 


## 1 Access

a. What kind of outdoor/natural spaces do people have access to (Q2.3)? 
b.what quantity and level of diversity (ie higher quantity of spaces) of access do they have Q2.3)?

Access: 
ACCESS TOTAL – Recode ‘none of the above as 0, rest as ‘1’s – sum  total number of spaces checked off per person
OUTDOORS AT HOME: Combine ‘private or shared yard’ with ‘deck balcony or patio’ – if answered yes to one of these recode as ‘1’ if answered yes to both recode as ‘2’, if didn’t check either recode as ‘0’
OUTDOORS NEAR HOME – recode as the number of boxes checked other than the 2 in outdoors at home; if checked ‘other’ count as 1
DIVERSITY NEAR HOME More than 3 spaces outdoors near home – create a binary category – if checked off 3 or more boxes in the ‘outdoors near home’ group
...
c. Does access (multiple variables) correlate with type of home setting (Q7.7)

outdoorsathome and neighbortype are highly correlated: urban neighborhood and number of types of ourdoorsathome are signifiacantly negative correlated.
...
Q1.1<-xtabs(~accessscore + neighbortype,data=df.chi)
Q1.1
CMHtest(Q1.1)
...
c1. Is access a moderator b/w use or time outdoors and neighborhood type?
For this question, it is not very clear what access indicates here.
So I simplify this question:
What is the relationship between time outdoors and neighborhood type?
Time
Time.daysperweek:
...
There is correlation between time ourtdoors perweek and neighborhood.
...
d.	Does access at or near home OR diversity of spaces near home correlate to TOTAL TIME outdoors (in any of the 3 time periods)
...
e.	Access change pre to post covid – within subjects and across subjects 
This question is actually asking:
1. Use change pre to post covid within subjects significant?
2. Use change pre to post covid in total significant?
We answer these 2 questions below in Use section.


## 2 Use

a. Use change pre to post covid within subjects significant?
We use paired-test and treat the total number of types of place used as a continous variable. The test showed after COVID, space use is significantly less (0.19, p = 0.03)
...
b. Use change pre to post covid in total significant?
...
c.	Does access relate to use (Q2.4) – correlation between TOTAL TIME outdoors (Fall 2020) and: 
i.	Total number of spaces they can access (ACCESS TOTAL from Q2.3)
ii.	Whether they have space ‘at home’ (new variable)
iii.	Total number of spaces ‘outdoors near home’ (new variable)
iv.	Whether they have 3 or more spaces outdoors near home (diversity near home - new variable) 

Here, this question is confusing: I think the question is:
Does access relates to use - correlation between the 4 access variables with total number of used places before and after- 8 correlations.
```{r}
...
CMHtest(Q2.c6)
Q2.c7<-xtabs(~df.chi$accessscore+Q2.4.score$use.score.before)
CMHtest(Q2.c7)
Q2.c8<-xtabs(~df.chi$accessscore+Q2.4.score$use.score.after)
CMHtest(Q2.c8)


```

d.	Compare each above to time outdoors since covid emerged (Q2.8) and in Fall 2019 (Q2.10)
...
e.	Does use (ie time outdoors) correlate with any of the following: 
...
```
f.	Changes in use of spaces pre to post covid – within and across subjects
```{r}
...
a. Did staff typically spend time outdoors during the workday (Q5.2)? 
```{r}
raw0 %>% 
...
b. what did they typically do (Q5.6)
```{r}
Q5.6 %>% 
  pivot_longer(eat:others,
               names_to = "activities",
...
c. time outdoors correlation with: 
...
xv.	TOTAL CAMPUS ACTIVITY 

```{r, class.output="scroll-100"}
ltm::rcor.test(df.chi2,use="pairwise.complete.obs")
```

d. More or less time outdoors than when working from home?
```{r}
raw %>% 
...
e. More or less than on campus pre-covid (Q5.5)
...
```
b.	Time (both) is correlated with total barriers – very interesting – dig a bit deeper in relation to barriers – do barriers change by nbhd type? Gender? Attitudes towards nature?
b1. Time.daysperweek and Time.hourspertime and total barriers:
Time.daysperweek NOT correlated with total barriers;
...
b2. do barriers change by nbhd type? Gender? Attitudes towards nature?
```{r}
...
c.	Did barriers change from pre to post covid

I do not know where to find the coresponding question in the survey.
```{r}

```

## 5 Other questions
a.	Who do they spend time with in nature (at home and at work and for both pre and post covid) and how did it change – within and across subjects?

Total types of companions are significantly fewer.
For each type of companion, the before and after are still correlated.
Workmates and friends are significantly decreasing during COVID.
...
```
b.	What do they do outdoors (activities) - (at home and at work and for both pre and post covid) and how did it change – within and across subjects?

Q5.6 is about what typicaly do on campus
...
Q 3.2 is asking what activities typically do before and during covid?
...
c.	Why do they spend time outdoors in nature, and how did this change during covid (Q4.2)
c1.	Correlation between WHY and time outdoors
c2.	How do reasons for spending time outdoors intersect with:
...
d.	Is there a connection between people’s motivation for  spending time in nature (Q4.2) attitudes towards nature (Q4.4) OR perceived benefits (Q4.3) and time spent outdoors [both per and post covid] (Q2.5 to 2.10) – does this differ for any demographic groups
...
e.	By neighborhood type are there changes in the number and type of places used pre to post covid, and within and across subjects
...
f.	Is there increased contact with nature as a result of covid?
...
g1.	What does this intersect with – attitudes?
